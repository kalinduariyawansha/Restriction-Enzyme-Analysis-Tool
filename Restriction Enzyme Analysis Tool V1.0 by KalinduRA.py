...
if st.session_state.get("run_analysis", False):
    ...
    if len(available_enzymes) == 0:

        st.warning(
            "No restriction enzyme sites were found in the uploaded sequence."
        )

        st.stop()

    if (
        "selected_enzyme" not in st.session_state
        or st.session_state.selected_enzyme not in available_enzymes
    ):
        st.session_state.selected_enzyme = available_enzymes[0]

    enzyme_search = st.text_input(
    "Search Enzyme"
    )

    filtered_enzymes = [
        e for e in available_enzymes
        if enzyme_search.lower() in e.lower()
    ]
